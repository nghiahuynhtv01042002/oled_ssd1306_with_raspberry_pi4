...
def resize_and_convert_to_bitmap(frame):
    # Resize image to 128x64
    resized_img = cv2.resize(frame, (128, 64), interpolation=cv2.INTER_AREA)
...
def main():
    ...
    gif = cv2.VideoCapture('catgif.gif')
    ...
    while True:
        ...
        # Resize and convert to bitmap
        bitmap = resize_and_convert_to_bitmap(gray_frame)
